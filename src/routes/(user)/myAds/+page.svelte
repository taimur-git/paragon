    ...
    <div class="flex flex-wrap allad">
      {#each ads as ad}
        <!-- {#if selectedTags.length === 0 || selectedTags.some(tag => ad.tags.includes(tag))} -->
          <article class="m-2" on:click={() => modal.show(ad)}>
            <Card>
              <div slot="header">
                Name: {ad.user}
              </div>
              ...
              <div slot="buttons" class="buttons">
                <form method="POST">
                <button class="fixed-button bg-green-500 hover:bg-green-700 text-white font-semibold py-2 px-4 rounded-md see-button" >
                  Delete
                </button>
